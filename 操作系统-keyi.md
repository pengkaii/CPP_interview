...
malloc() 并**不是系统调用**，而是 C 库里的函数，用于动态分配内存。
...
1. **多任务操作**：当当前进程的时间片耗尽时，操作系统需要将CPU分配给其他进程，此时会进行上下文切换。
2. **等待I/O或系统调用**：如果一个进程进行了I/O操作或系统调用并需要等待结果，那么它将被挂起，操作系统会切换到另一个就绪的进程。
3. **进程阻塞**：类似于线程的阻塞，进程也可能因为等待某个资源（如文件、网络资源、信号量等）而被阻塞。
4. **高优先级进程就绪**：如果一个高优先级的进程变得就绪（例如，它正在等待的资源变得可用），操作系统可能会预empt当前正在运行的较低优先级进程。
5. **中断处理**：当处理器接收到一个中断信号时，可能会暂停当前进程以响应中断。
6. **内存管理**：为了防止某一进程过多地占用物理内存导致其他进程无法正常工作，操作系统可能会根据某些策略（如内存页面替换算法）来进行上下文切换。
7. **用户交互**：在用户交互式系统中，为了响应用户的操作，操作系统可能会切换到用户正在交互的进程。
8. **程序执行完毕或手动终止**：当一个进程执行完毕或者被操作系统或用户手动终止时，操作系统需要从就绪队列中选择一个新的进程来执行。
9. **负载平衡**：在多处理器系统中，为了维持处理器之间的负载平衡，操作系统可能会在处理器间迁移进程。
...
线程上下文切换是指操作系统在CPU上切换不同线程的执行的过程。每当操作系统决定停止执行当前运行的线程，并开始执行另一个线程时，就会发生上下文切换。这个过程发生在多线程环境中，是多任务操作系统中实现时间共享的一种机制。上下文切换确保CPU的运算能力可以被系统中所有的线程公平地使用。
...
1. **时间片用完**：大多数操作系统使用时间片（quantum）或时间轮（round-robin scheduling）来确保所有的线程都有机会执行。当一个线程的时间片用完时，操作系统会强制它让出CPU，然后选择另一个线程来执行。
2. **阻塞操作**：当一个线程进行了一个需要等待的操作，比如I/O操作、系统调用或是等待某个资源（如锁）时，操作系统会将其置于等待状态，并进行上下文切换到另一个就绪状态的线程。
3. **线程优先级**：当一个高优先级的线程变为就绪状态时，如果它比当前执行的线程优先级高，操作系统可能会进行上下文切换，以保证高优先级的任务能更快地执行。
4. **中断处理**：当CPU接收到外部中断信号时，比如来自硬件设备的信号，操作系统可能会暂停当前线程的执行来处理中断，然后再回到线程执行或是切换到另一个线程。
5. **同步构造**：线程在尝试获取一个已经被其他线程持有的锁时，将会被阻塞，此时操作系统也会进行上下文切换。
6. **多处理器调度**：在多处理器或多核心系统中，线程可能在不同的处理器或核心之间迁移，以保持负载平衡，这时也会发生上下文切换。

上下文切换是一项昂贵的操作，因为它涉及到保存和恢复线程的状态，包括程序计数器、寄存器集和内存映射等。因此，在设计系统时，通常会试图减少上下文切换的次数来提高效率。
...
中断上下文切换是操作系统响应中断信号时发生的一种特殊类型的上下文切换。在计算机系统中，中断是硬件或软件产生的信号，它告诉处理器立即停止当前的工作，并去执行与中断相关的代码。中断上下文切换不同于进程或线程的上下文切换，它通常涉及到内核态的切换和对处理器状态的保存和恢复。

中断上下文切换场景：

1. **硬件中断**：来自硬件设备的信号，例如键盘输入、鼠标移动、网络数据包到达，或者时钟中断。处理器需要立即响应这些外设的请求。
2. **软件中断**：也被称为异常或陷阱，例如一个程序执行了非法操作（如除零错误），或者系统调用（如文件操作请求）。
3. **同步中断**：当正在执行的指令主动请求中断服务，如系统调用或执行了特定的中断指令。
4. **非屏蔽中断（NMI）**：这是一种特殊的硬件中断，不能被屏蔽，通常用于处理紧急情况，如硬件错误。

当中断发生时，CPU当前的状态会被保存在栈中，以便中断处理完成后能够恢复执行原来的任务。操作系统的中断服务例程（Interrupt Service Routine, ISR）会接管控制权，处理中断信号。中断处理通常包括以下步骤：

1. **中断发生**：硬件信号到达CPU，CPU响应信号并根据中断向量表找到相应的服务例程。
2. **保存状态**：CPU自动保存关键的寄存器内容，如程序计数器（PC）和状态寄存器，到栈中。
3. **执行ISR**：中断服务例程执行，处理中断信号。
4. **恢复状态**：ISR完成后，保存的状态从栈中恢复，CPU返回到被中断的程序继续执行。

中断上下文切换是一种非常快速的操作，它允许操作系统几乎立即响应外部事件。不过，即使是这种快速的上下文切换也有一定的处理开销，所以中断处理代码通常需要高度优化，以减少对系统性能的影响。
...
内存管理单元（MMU）负责虚拟地址到物理地址的转换，允许操作系统为每个进程提供一个大而连续的地址空间，而物理内存可能是分散和有限的。

1. **生成虚拟地址**：
   - 当程序需要访问内存时，它会生成一个虚拟内存地址。这个地址是相对于进程的虚拟地址空间的，而不是物理内存的直接地址。
   - MMU使用页表来将虚拟地址转换成物理内存地址。
2. **查找转换后备缓冲器（TLB）**：
   - MMU首先检查TLB，这是一个包含了部分页表条目的缓存。如果找到了虚拟地址对应的条目（TLB命中），则直接进行到下一步。如果没有找到（TLB未命中），则需要访问页表。
3. **访问页表**：
   - MMU使用虚拟地址中的一部分（如页号）作为索引，来访问页表，找到相应的页表项。
   - 页表通常存储在内存中，但为了高效访问，部分内容会缓存在TLB中。
   - 页表包含了虚拟地址和物理地址映射的信息，通常由操作系统维护
4. **检查页表项**：
   - 页表项包含了该虚拟页对应的物理页框号以及一些控制位（如是否在内存中、访问权限等）。
   - 如果页表项表明该页不在物理内存中（即页面错误或缺页），则会触发一个缺页中断，由操作系统来处理（如从磁盘中加载页面到物理内存）。
   - 页表可能被分成多级，以减少所需的存储空间。
5. **计算物理地址**：
   - 一旦有了物理页框号，MMU将它与虚拟地址中的页内偏移组合，形成完整的物理地址。
   - 页内偏移是虚拟地址的一部分，表示在页面内的具体位置。
6. **访问物理内存**：
   - 现在，MMU已经有了物理地址，CPU可以使用这个地址访问实际的物理内存。
...
CPU只需要告诉MMU需要转换的虚拟地址是什么，然后由MMU进行查询
...
虚拟内存是一种内存管理技术，它通过使用磁盘空间来模拟额外的内存资源，从而使得操作系统和应用程序能够超出物理内存的限制使用内存。这样做的好处是可以增加程序的地址空间，提高物理内存的使用效率，并允许更大的程序运行，同时还可以通过内存的分页或分段机制来为每个进程提供独立的内存空间，增强安全性和稳定性。虚拟内存的管理依赖于操作系统、硬件中的内存管理单元（MMU）和一系列内存管理策略。

**虚拟地址到物理地址转换过程**

**1. 分段（Segmentation）**

- **分段转换**：在某些体系结构中，第一步是通过分段机制来进行。每个段（比如代码段、数据段等）有一个基址和限长，虚拟地址首先被划分为段号和段内偏移。
- **地址检查**：系统检查段内偏移是否超出了限长，如果超出则产生错误。
- **地址转换**：如果没有错误，系统将段内偏移加上段的基址得到一个线性地址或直接的物理地址（取决于系统是否还采用分页）。

**2. 分页（Paging）**

- **线性地址到物理地址**：现代操作系统通常使用分页。线性地址（或者在不使用分段的系统中的虚拟地址）被分成多个部分：页号（或页表索引）和页内偏移。
- **页表查找**：系统使用页号作为索引去查找页表条目。这个页表条目包含了物理页帧的地址。
- **访问页表**：如果该页表条目存在于内存中（即页在内存中），则直接使用；如果不在，会发生页错误（page fault），操作系统必须从磁盘（或其他存储介质）中加载该页到内存中。
- **地址转换**：物理页帧地址与页内偏移结合形成完整的物理地址。

**3. TLB（Translation Lookaside Buffer）**

- **TLB缓存**：因为访问页表需要时间，现代处理器使用一种叫作TLB的高速缓存，它存储最近使用的页表条目。
- **快速查找**：当一个虚拟地址需要转换时，处理器首先查看TLB是否有这个地址的映射。如果TLB命中，那么物理地址可以直接被获取，大大加快了转换速度。
- **TLB未命中**：如果TLB没有命中，那么处理器必须去读取实际的页表，这个过程比较慢。

**4. 页错误处理（Page Fault Handling）**

- **页错误**：当一个需要的页不在物理内存时，发生页错误。
- **操作系统介入**：此时操作系统会暂停当前程序的执行，将缺失的页从磁盘加载到内存中，更新页表，并重新执行导致页错误的指令。



### **作用**

- **内存抽象**: 虚拟内存为每个进程提供了一个连续的地址空间，屏蔽了物理内存的复杂性和局限性。这意味着即使物理内存被分散地使用，每个进程仍然能看到一个连续的内存空间。
- **内存扩展**: 通过将磁盘空间用作虚拟内存，系统可以提供比物理内存更大的地址空间。这使得运行大型程序或多个程序同时运行成为可能，即使可用的物理内存较小。
- **内存隔离**: 每个进程有自己的虚拟地址空间，与其他进程隔离。这提供了一种安全机制，防止一个进程访问或修改另一个进程的内存。
- **内存保护**: 通过虚拟内存，操作系统可以对不同区域的内存设置不同的访问权限（如只读、可写等），以防止程序错误或恶意软件破坏系统稳定性。
- **简化内存管理**: 虚拟内存简化了内存分配和回收的过程，操作系统可以更灵活地管理内存资源。
...
- 虚拟内存是计算机系统内存管理的一种机制，它通过使用硬盘存储来扩展可用的RAM（随机访问存储器）空间。通过这种方式，操作系统可以为运行中的程序提供比物理内存更大的地址空间，使得它们能够使用比实际可用的RAM更多的内存资源。
- 虚拟内存工作的基本原理是将程序的地址空间分割成固定大小的块，称为“页”（pages）。这些页被映射到物理内存或硬盘上的空间。当程序尝试访问一个地址时，虚拟内存系统会检查这个地址对应的页是否在物理内存中。如果在，就直接访问。如果不在，操作系统会从硬盘中调入所需的数据，可能将其他不再需要的数据页调出到硬盘以腾出空间。
- 这个过程是对用户和应用程序透明的，他们感知到的是一个连续的、较大的内存空间，而不是实际的物理内存大小。虚拟内存的使用使得运行大型应用程序、支持多任务等操作成为可能，提高了系统的灵活性和效率。
- 虚拟内存还提供了内存保护和隔离的功能，保证了每个进程都有自己独立的地址空间，不能直接访问其他进程的内存，这增强了系统的稳定性和安全性。
...
线程是操作系统调度的最小单位。它被包含在进程之中，是进程中的实际运作单位。在单个程序中可以同时运行多个线程完成不同的任务，特别是在多核处理器上。
...
**轻量级进程（*LWP*）是内核支持的用户线程，一个进程可有一个或多个 LWP，每个 LWP 是跟内核线程一对一映射的，也就是 LWP 都是由一个内核线程支持。**
...
**3.忽略信号**。当我们不希望处理某些信号的时候，就可以忽略该信号，不做任何处理。有两个信号是应用进程无法捕捉和忽略的，即 `SIGKILL` 和 `SEGSTOP`，它们用于在任何时候中断或结束**某一进程**。



#### Socket通信

**跨网络与不同主机上的进程之间双向通信**，一个Socket由IP地址和端口号唯一确定。Socket 通信不仅可以跨网络与**不同主机**的进程间通信，还可以在**同主机**上进程间通信。
...
1. **流式Socket（Stream Socket）**：
   - 通常使用TCP协议提供面向连接的稳定数据流服务。
   - 保证数据正确顺序且无重复。
   - 适用于要求可靠传输的应用，如文件传输、HTTP/FTP协议等。
2. **数据报Socket（Datagram Socket）**：
   - 通常使用UDP协议提供面向消息的服务。
   - 数据可能丢失或不按顺序到达。
   - 适用于不需要建立连接，对网络通讯性能要求较高的场合，如多媒体内容的传输。

**Socket通信的基本流程**

1. **服务器端**：
   - 创建Socket：`socket()`
   - 绑定Socket到一个地址和端口上：`bind()`
   - 开始监听连接：`listen()`
   - 接受连接请求：`accept()`
   - 通过返回的Socket进行读写操作：`read()/write()` 或 `recv()/send()`
   - 关闭连接：`close()`
2. **客户端**：
   - 创建Socket：`socket()`
   - 发起连接请求到服务器指定的地址和端口：`connect()`
   - 通过Socket进行读写操作：`read()/write()` 或 `recv()/send()`
   - 关闭连接：`close()`
...
- type 参数用来指定通信特性，比如 SOCK_STREAM 表示的是**字节流**，对应 TCP、SOCK_DGRAM 表示的是**数据报**，对应 UDP、SOCK_RAW 表示的是原始套接字；
- protocal 参数原本是用来**指定通信协议的**，但现在基本废弃。因为协议已经通过前面两个参数指定完成，protocol 目前一般写成 0 即可；



Socket通信的特点

- **支持不同网络协议**：虽然Socket最初设计用于TCP/IP协议，但它也可以用于其他网络协议的通信。
- **跨平台性**：Socket API在不同的操作系统之间具有一致性，方便跨平台编程。
- **双向通信**：Socket可以实现全双工通信，即数据可以在两个方向上同时传输。
- **阻塞与非阻塞模式**：Socket可以工作在阻塞模式和非阻塞模式，这影响着数据的发送和接收方式。
- **支持异步操作**：现代操作系统提供了异步Socket操作的API，比如`select()`、`poll()`和`epoll()`等。

使用场景

Socket广泛用于互联网应用程序中，如：

- 网络服务，如Web服务器和客户端之间的HTTP通信。
- 邮件传输，如SMTP、POP3和IMAP协议。
- 文件传输，如FTP客户端和服务器之间的通信。
- 实时通信，如在线游戏、聊天应用等。
...
对于**本地字节流 socket**，其 socket 类型是 AF_LOCAL 和 SOCK_STREAM。

对于**本地数据报 socket**，其 socket 类型是 AF_LOCAL 和 SOCK_DGRAM。
...
由于多线程执行操作**共享变量的这段代码**可能会导致竞争状态，因此我们将此段代码称为**临界区，它是访问共享资源的代码片段，一定不能给多线程同时执行。**
...
互斥解决了**并发进程/线程对临界区的使用问题**。这种基于临界区控制的交互作用是比较简单的，只要一个进程/线程进入了临界区，其他试图想进入临界区的进程/线程都会被阻塞着，直到第一个进程/线程离开了临界区。
...
这两个都可以方便地实现进程/线程互斥，而**信号量比锁的功能更强**一些，它还可以方便地实现进程/线程同步。
...
使用**加锁操作和解锁操作**可以解决并发线程/进程的互斥问题。

任何想进入临界区的**线程**，必须先执行**加锁**操作。若加锁操作顺利通过，则线程可进入临界区；在完成**对临界资源的访问后再执行解锁操**作，以释放该临界资源。
...
既然不想自旋，那当没获取到锁的时候，就把**当前线程放入到锁的等待队列**，然后执行调度程序，把 CPU 让给其他线程执行。



##### 信号量

信号量是操作系统提供的一种**协调共享资源访问**的方法。
...
它用于创建一个新的进程。这个新创建的进程被称为子进程，它是父进程的一个复制品。`fork()`函数调用后，父进程和子进程将会从`fork()`调用之后的下一条指令开始执行。

1. **进程复制**: 当一个进程调用`fork()`时，操作系统会创建一个新的进程。这个新进程几乎是原进程的完全复制（包括代码段、数据段和堆栈段）。新进程获得了与父进程相同的内存映像、相同的打开文件描述符、相同的环境变量和相同的程序计数器状态等。
2. **分配PID**: 新进程被赋予一个唯一的进程标识号（PID）。
3. **返回值**: 在父进程中，`fork()`返回新创建子进程的PID。在子进程中，`fork()`返回0。这是区分父进程和子进程的一个重要手段。
4. **独立执行**: 一旦`fork()`调用完成，父进程和子进程将独立执行，它们拥有独立的地址空间，对变量的修改不会相互影响。
5. **共享文件描述符**: 尽管父进程和子进程在内存中是分离的，但它们会共享打开的文件描述符。这意味着，如果父进程和子进程都打开了一个文件，它们可以互相影响文件的读写位置。
...
1. 对于**CPU密集型**的程序，多线程并不能提高运行效率
2. 对于**IO密集型**的程序，多线程可以提高运行效率。
...
线程间的同步方式包括**信号、信号量、条件变量、锁机制**：

1. **信号**：类似进程间的信号处理
2. **信号量**：计数器，允许多个线程同时访问同一个资源。
3. **条件变量**：通过条件变量通知操作的方式来保持多线程同步。
4. **锁机制**：采用互斥对象机制，只有拥有互斥对象的线程才可以访问。因为互斥对象只有一个，所以可以保证公共资源不会被多个线程同时访问。
...
- 在 Linux 下，我们可以使用 `pstack` + `gdb` 工具来定位死锁问题。pstack 命令可以显示每个线程的栈跟踪信息（函数调用过程），它的使用方式也很简单，只需要 `pstack <pid>` 就可以了。在定位死锁问题时，可以多次执行 pstack 命令查看线程的函数调用过程，多次对比结果，确认哪几个线程一直没有变化，且是因为在等待锁，那么大概率是由于死锁问题导致的。不能够确认这两个线程是在互相等待对方的锁的释放，因为看不到它们是等在哪个锁对象，于是可以使用 gdb 工具进一步确认。
...
CAS 是乐观锁没错，但是 CAS 和自旋锁不同之处，自旋锁基于 CAS 加了while 或者睡眠 CPU 的操作而产生自旋的效果，加锁失败会忙等待直到拿到锁，**自旋锁**是要需要事先拿到锁才能修改数据的，所以算**悲观锁**。
