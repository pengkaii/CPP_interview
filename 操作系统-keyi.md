...
由于多线程执行操作**共享变量的这段代码**可能会导致竞争状态，因此我们将此段代码称为**临界区（*critical section*），它是访问共享资源的代码片段，一定不能给多线程同时执行。**
...
### 进程调度算法(FCFS、SJF、HRRN、RR、HPF、MFQ)
...
### 内存页面置换算法（最佳页面置换、先进先出置换、LRU、Clock、LFU）
