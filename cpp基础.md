...
递归过程的局部变量过多、递归深度过大，是造成系统栈溢出的原因，特别是递归列循环时肯定会发生系统栈溢出，栈的大小通常是1M-2M,所以栈溢出包含两种情况，一是分配的的大小超过栈的最大值，二是分配的大小没有超过最大值，但是**接收的buf比原buf小**。
...
46. `diff`：比较文件之间的差异。
47. `zip`：创建 ZIP 归档文件。
48. `unzip`：解压缩 ZIP 归档文件。
49. `ssh-keygen`：生成 SSH 密钥对。
50. `rsync`：远程数据同步。
51. `alias`：创建命令别名。
