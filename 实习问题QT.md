...
**发送消息API：**由于不在CAN线程下的run函数中执行，所以不在CAN线程中。CAN线程收到机械臂末端到达位置里程点确认信息之后，则向下位机控制器发送臂架矩阵信息，监测传感器数据以判断臂架是否停好，之后执行相机线程canmera_thread。

**CAN线程：**位置确认信号——CAN线程响应。（无需信号槽，内部调用即可）
