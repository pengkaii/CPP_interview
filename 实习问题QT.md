...
`QThread` 和 `std::thread` 都是提供线程功能的类，但它们分别来自两个不同的库和背景，因此有一些关键的差异。

1. **来源与应用背景**:
   - `QThread`: 来自 Qt 库，一个跨平台的应用程序框架，主要用于图形用户界面(GUI)应用，但也可以用于其他类型的应用。
   - `std::thread`: C++11 标准库的一部分，是一个通用的线程类。
2. **生命周期与所有权**:
   - `QThread`: 在 Qt 的事件循环中有特殊的地位。它可以与一个 `QObject` 关联，并接收 Qt 信号和槽的消息。它的生命周期通常由 Qt 的对象模型管理。
   - `std::thread`: 更为底层和简单。它表示一个操作系统级的线程，没有额外的事件循环或消息系统与之关联。
3. **创建与启动**:
   - **`QThread`: 通常通过重写 `run` 方法来定义线程的行为。要启动线程，需要调用 `start` 方法。**
   - **`std::thread`: 在构造时通过传递一个函数对象、Lambda 函数、函数指针等来定义线程的行为。**
4. **与 GUI 交互**:
   - `QThread`: Qt 不允许从非 GUI 线程访问 GUI 组件。但你可以使用 Qt 的信号和槽机制在不同的线程之间进行通信，从而在主线程中更新 GUI。
   - `std::thread`: 由于它是一个通用的线程类，因此与特定的 GUI 系统的交互方式因库而异。
5. **同步与通信机制**:
   - **`QThread`: 除了常规的互斥和条件变量外，Qt 还提供了信号和槽的机制，允许在线程之间进行安全的通信。**
   - **`std::thread`: C++11 提供了一系列的同步原语，如 `std::mutex`, `std::condition_variable`, 和 `std::atomic`.**
6. **终止线程**:
   - `QThread`: Qt 强烈建议不要强制终止线程，而是使用信号和槽的机制来通知线程应该退出。
   - `std::thread`: 也不应该强制终止线程。你应该为线程提供一个退出机制或使用合适的同步原语。
7. **依赖性与跨平台性**:
   - **`QThread`: 作为 Qt 库的一部分，使用它需要依赖 Qt，但 Qt 是跨平台的。**
   - **`std::thread`: 作为 C++ 标准库的一部分，它是跨平台的，但具体的行为可能受到底层操作系统的影响。**

总的来说，选择使用 `QThread` 还是 `std::thread` 通常取决于你的具体需求以及你是否已经在使用 Qt。如果你正在开发一个 Qt 应用，使用 `QThread` 可能更为合适。如果你正在写一个不依赖特定框架的 C++ 程序，`std::thread` 可能是更好的选择。


## **还有信号槽，怎么自己实现**

我回答类比于Linux下的信号与信号处理函数的机制

## 信号和槽

### 1. 信号（Signals）

- 信号是在 Qt 中某个特定事件发生时被发射（emit）的。
- 信号没有返回值。
- 是类的成员函数，但不需要在类的实现文件中定义，只需要在头文件中声明。
- 可以有参数，用于传递信号的数据。

例如，当一个按钮被点击时，`QPushButton` 类发射一个 `clicked()` 信号。

### 2. 槽（Slots）

- 槽是为了响应信号而执行的函数。
- 可以有返回值（但是返回值不能被用到信号-槽连接中）。
- 是类的普通成员函数，需要在头文件中声明，在实现文件中定义。
- 一个槽可以连接到任意数量的信号。
- 槽函数可以直接调用，就像普通的 C++ 函数一样。

### 3. 连接（Connect）

为了响应某个信号，你需要将该信号连接到一个槽。这可以通过 `QObject::connect` 函数完成：

```C++
connect(sender, SIGNAL(signalName(parameters)), receiver, SLOT(slotName(parameters)));
```

当 `sender` 发射了 `signalName` 信号时，`receiver` 的 `slotName` 槽函数就会被调用。

### 例子：

假设我们有一个按钮和一个标签，当按钮被点击时，我们想在标签上显示 "Button Clicked!" 的消息：
...
1. ⚠️**松散耦合**：发送者和接收者类可以独立修改，不会相互影响。这意味着你可以改变一个对象的实现，而不影响到与其连接的其他对象。
2. **代码简洁明了**：使用信号和槽可以使代码更加清晰和组织化，因为你可以直接看到哪些函数是为响应特定信号而设计的。
3. **类型安全**：从 Qt 5 开始，新的信号和槽语法提供了更好的编译时类型检查，这有助于减少运行时错误。
4. **灵活性**：
   - 一个信号可以连接到多个槽。
   - 一个槽可以连接到多个信号。
   - 信号可以链接到其他信号。
5. **参数传递**：信号可以携带参数，这些参数可以传递给槽函数，使得响应更具上下文信息。
6. **跨线程通信**：信号和槽机制还支持跨线程的对象通信，使得在多线程应用程序中同步数据和事件变得简单。

### 缺点：

1. **性能开销**：与直接调用函数相比，信号-槽连接有一些运行时开销。然而，在绝大多数情况下，这种开销是可以接受的。
2. **编译时间增加**：由于 Qt 使用 moc（Meta-Object Compiler）来处理信号和槽，这可能会稍微增加编译时间。
3. **学习曲线**：对于那些不熟悉 Qt 的开发者，信号和槽机制可能需要一些时间来理解和掌握。
4. **运行时错误难以追踪**：在旧的信号和槽语法中，如果信号或槽的签名改变了，但连接语句没有相应地更改，这可能会导致运行时错误，而不是编译时错误。
5. **过度使用可能导致代码难以维护**：如果在一个项目中大量使用信号和槽，可能会使得数据流和事件流变得难以跟踪和理解。

总的来说，Qt 的信号和槽机制在设计和实现事件驱动的应用程序时非常有用。当正确使用时，它可以提供强大的功能和出色的代码组织。然而，像所有工具一样，最好在合适的场合中使用它。
...
（1）对六自由度喷浆机械臂进行运动学分析，基于D-H法建立正运动学模型，通过各关节的变化量得到末端喷头的实时位置和姿态。基于正运动学模型，采用约束关节的解析法进行逆运动学求解，通过末端喷嘴的位姿可求到各关节的目标关节量，因为该喷浆机械臂采用液压缸直接驱动方式，机械臂运动关节数量最少是符合最优解。

（2）针对隧道的受喷面，规划出扫面和填充工序的喷嘴运动轨迹规划，完成了基于工人经验的喷浆参数的设置。在喷嘴运动运动轨迹基础上，对六个关节进行了多项式插值对比实验，验证五次多项式在喷浆机械臂的轨迹规划优势，满足冲击最优算法。

（3）通过拉格朗日法推导机械臂的动力学方程，并利用SimMechanics建立动力学模型，以验证动力学方程的准确性。基于对滑模控制的滑模面、趋近率以及传统指数滑模算法的问题分析，自适应模糊滑模控制算法，通过引入新的滑模面和模糊系统来处理外部干扰和切换函数，从而实现干扰抑制和系统稳定。通过应用Lyapunov稳定性分析方法，作者证明了该新型滑模控制算法的稳定性。最后，作者使用机械臂的动力学方程进行了仿真试验，将新型滑模控制算法与传统指数滑模控制算法进行对比。仿真结果表明，新型滑模控制算法在关节位置和速度跟踪性能方面表现更优，并且能够有效消除控制输入的小范围高频抖动现象，具有更强的抗干扰性和稳定性。

（4）基于Qt开发了一款人机交互性强的上位机软件，并运用面向对象编程和结构化技术，将运动学、动力学、轨迹规划和跟踪控制等研究内容集成到机械臂控制软件中。根据软件架构设计，分别设计和实现了GUI模块、通讯模块和模式切换模块。在喷浆机械臂试验平台上进行了现场实验，得到了六个关节的跟踪曲线和喷嘴在三维坐标系中的误差，并满足自动化喷浆机械臂行业的要求。实验结果表明，本文提出的喷浆机械臂轨迹规划和新型自适应模糊滑模控制器方法具有有效性和实用性。

### 前置理解：

上位机只跟控制器通信，控制器再输出电气io控制液压阀再控制机械臂。
...
传感器：***\*臂架姿态的精确获取\****是实现臂架智能运动控制的前提，因此在臂架加装有6个旋转编码器、2个拉线传感器。
...
在GUI编程中，***\*QWidget类提供了渲染到屏幕以及处理用户输入事件的基本类。 Qt提供的所有UI元素都是QWidget的子类。 通过继承QWidget重新实现虚拟事件处理程序来完成创建自定义窗口小部件。\****当更改一个widget(***\*所有用户界面对象的基类\****）时候，希望通知另一个widget。

在Qt中使用***\*信号和槽\****替代回调技术。特定事件发出信号，***\*有槽函数响应信号\****从而调用函数。***\*信号的签名必须和槽的签名相匹配\****，编译器运行时候帮助检查字符串的SIGNAL和SLOT语法类型匹配。发出信号的类不知道和哪个槽函数连接，

所有信号和槽是松耦合。Qt的信号和槽机制可以保证将信号连接到槽后，***\*槽将在正确的事件用信号参数调用槽函数\****。使用QObject::connect()函数调用实现***\*信号和槽函数的连接\****。信号和槽提高了开发效率，保证界面和数据交互的通讯安全。

#### OpenGL模块

GUI模块使用了Solidworks软件对喷浆机械臂进行了建模，并为其附加了零件材质，保存为IGS格式，并用3ds Max软件进行模型缩放，最后导出为obj格式。通过***\*使用OpenGL类库中的glRotatef和glTranslatef函数对每个臂架的坐标系位置和方向进行了改变，以实现对机械臂运动的控制和显示。通过load函数，我们将obj文件导入Qt的OpenGL中，\****从而实现了对***\*机械臂的三维模型的控制和显示\****。

#### **通讯模块**

主要用来传输传感器的数据、遥控器手柄信号、上下位机之间的通信。

上位机软件逆解**6个关节角度的运动控制信息、三种模式切换信号通过CAN通信给控制器。\****

***\*控制器将传感器检测到的6个关节角度，遥控器数据（按钮、摇杆）通过CAN线发送给上位机。**
...
**发送接口函数**是将**待发送的CAN消息**（关节传感器数据）添加到发送队列中，定时将其发送到总线上，以避免消息的丢失或冲突。

***\*接收接口函数\****是将接收到的数据放入接收队列**中，作为一个缓存区来存储接收到的CAN数据帧，接收队列通常采用循环队列或链式队列**等数据结构实现，使用***\*先进先出的原则\****进行数据帧的存储和管理。CAN总线可能接收到多个节点的数据帧，因此使用接收队列可以帮助系统缓存并按顺序处理这些数据帧，避免数据的丢失或冲突。
...
CAN总线只定义了物理层和数据链路层，而CANopen是基于CAN总线的应用层协议。在CANopen应用层，设备之间通过交换通信对象进行通信。CANopen的通信对象类型中有两个对象用于数据传输。它们使用两种不同的数据传输机制来实现。
  ...
  #### can有很多节点，都往总线上发消息，总线怎么处理这个事情,怎么做仲裁
  CAN (Controller Area Network) 是一个为汽车和其他环境设计的健壮的、串行的通讯总线。当多个节点（也称为设备或ECUs）尝试同时在CAN总线上发送消息时，会出现竞争条件。为了解决这个问题，CAN协议定义了一个仲裁机制来确定哪个消息具有优先权。下面是该机制的简化概述：

基于优先级的仲裁：CAN的仲裁机制是基于消息ID的。较低的ID意味着较高的优先级，因此在仲裁过程中，低ID的消息会赢得总线访问权。

非破坏性的仲裁：这意味着当一个节点检测到其正在发送的消息与另一个节点的消息冲突时，它会停止发送，而不会影响或破坏总线上的其他消息。这样，高优先级的消息可以无阻碍地继续其传输。

仲裁过程：当两个或多个节点几乎同时开始发送消息时，它们首先发送它们的消息ID。每一个位都是按顺序从最重要的位（MSB）到最不重要的位（LSB）发送的。在发送每一个位时，每个节点都会监听总线上的电压，以确保它发送的是什么。

如果一个节点发送一个“0”（表示为低电平，也称为主动电平）和另一个节点发送一个“1”（表示为高电平，也称为被动电平），那么“0”会优先于“1”，因为CAN总线是线性或“与”型的。此时，发送“1”的节点会立即注意到这一点并停止发送，因为它已经知道其消息的优先级较低。

如果所有节点都发送相同的位，无论是“0”还是“1”，它们都会继续发送下一个位，直到其中一个节点发送一个与其他节点不同的位为止。

结束仲裁：一旦一个节点意识到其消息ID的某一位与总线上的电压不匹配（即，它正在尝试发送一个“1”，而总线上却是一个“0”），它就知道其消息的优先级较低，并立即停止发送。这样，具有最高优先级的消息可以继续其传输过程。

这种基于位的、非破坏性的仲裁机制确保了消息传输的高效性和确定性，使得实时系统中的关键消息能够在预期的时间内被传送。
...
工程机械臂的远程操作通常需要高度的注意力和手工灵活性。传统的自动化方法并不完全适用，因为不同的工作地点存在巨大差异性，过多的认知负载必然会导致疲劳，从而发生危险。一个熟练的机械臂操作员通过一些接口来操纵设备将混凝土喷射到开挖后的隧道岩石表面，隧道表面结构的不确定性，以及现场大量灰尘会阻碍操作者的视野。尽管现场环境恶劣，但操作员仍需保证现场施工安全，执行高效任务和最小成本化。

在传统的手动模式中，操作员需要直接在关节空间内提出指令，但人类是在三维坐标系下建立任务模型，因此必须快速和准确的进行机械臂逆运动学计算和发出指令，这样就造成高额的培训费用和较长的培训时间。

为了远程操作开发了各种接口，软件界面和人之间的互动（例如手柄、按钮、鼠标等）。工程机械领域通常以遥控器上的手柄和按钮来完成远程操作指令，喷浆机械臂遥控器如图5-7所示。喷浆机械臂的关节运动由控制器上的操纵杆控制，按钮通常用来定制一些需要的操作参数（例如，速凝剂流量、混凝土方量、刷动开关等）。

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps7.png) 
...
![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps9.png) 
...
针对目前关于自动化喷浆机械臂研究偏理论研究，进行实验验证分析的较少。本章基于运动学模型、轨迹规划、跟踪控制和自动控制系统，以GHP25C-IV型机械臂为实验设备，进行自动控制实验。在隧道模拟拱架下，规划出末端喷嘴运动轨迹，以该轨迹为理论运动轨迹，自动控制系统将控制机械臂按该轨迹运动，得到实际的关节跟踪曲线和喷嘴末端运动轨迹，并进行误差分析，验证所设计的喷浆机械臂控制系统的有效性。

### 5.3.2 硬件改装

机械臂加装旋转编码器UCD-CA01B-1213-L10S-PRM和拉线传感器GMX60R13/12E10RCB（9670A）。特点：绝对值码盘，高精度全数字化，位置与速度控制的双闭环，高速数字通讯输出等。传感器综合参数如表5-3所示：
...
| 3    | 线性分辨率      | 8192                            |
| 4    | 电源电压        | 9~30VDC                         |
| 5    | 抗干扰          | DIN EN 61000-6-2                |
| 6    | 单圈/多圈分辨率 | 13位/12位                       |
| 7    | 精度（INL）     | ±0.0878°（小于等于12位）        |
| 8    | 外壳防护等级    | IP66/IP67                       |
| 9    | 接口            | M12，公型，5针，A码             |
| 10   | 工作温度        | -25~85°C                        |
| 11   | 输出信号        | CANopen高速总线型，最高速率1MHz |
| 12   | 振动冲击        | 10g，10~2000Hz；100g，6ms       |

喷浆机械臂加装角度传感器和拉绳传感的如图5-12所示。关节1、关节2、关节3、关节5和关节6采用角度传感器测量关节转动角度，关节4采用拉绳传感器测出关节伸缩长度。

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps12.png) 
...
喷浆机械臂采用了丹弗斯PVG32比例阀，这是一种液压负载敏感比例阀，其主要功能是通过与负载相关的控制信号来实现对执行器的精确控制。相对于与负载无关的比例控制阀，该比例阀的使用可以有效地优化机械臂的性能，并且通过结合高性能执行技术，进一步提高机械臂的效率。在这个系统中，执行器的流量和压力控制是通过主阀芯位置在PVG工作部分来实现的。使用者可以通过比例阀上的操纵杆或控制器来控制机械臂的运动。PVG32比例阀原理图如图5-14所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps14.jpg) 

图5-14 PVG32比例阀

PVHC是用于主阀芯的电动执行控制模块，提供100·400Hz的脉宽调制（PWM）。在24v或48v电压下，比例阀阀芯行程和电流大小关系，如图5-15所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps15.png) 

图5-15 阀芯行程与电流

### 5.3.3 软件调试与喷浆机械臂控制实验

本文实验将笔记本电脑插上zlgCAN卡与喷浆机械臂的SYMC控制器的CAN0接口连接，以控制机械臂的运动，如图5-16所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps16.png) 

图5-16 机械臂硬件控制系统

上位机软件人机交互界面如图5-17所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps17.jpg) 

图5-17 上位机人机交互界面

现场试验在满足已经完成喷浆机械臂的结构加工和传感器安装基础上，现场试验环境如图5-18所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps18.png) 

图5-18 现场实验环境

基于对喷浆区域的理论轨迹规划，轨迹规划出模拟隧道作业区域的喷嘴运动曲线，如图5-19的红色曲线。

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps19.png) 

图5-19 模拟拱架和受喷区域的轨迹规划

**根据喷嘴末端的位姿运动轨迹，使用直线插补，计算出每个位姿点的各个关节值**，如表5-4所示：

表5-4 各关节位置变化

| 关节一(rad) | 关节二(rad) | 关节三(rad) | 关节四(m) | 关节五(rad) | 关节六(rad) |
| ----------- | ----------- | ----------- | --------- | ----------- | ----------- |
| 0.03341     | 0           | 1.5407      | 4.1164    | 1.5706      | -0.0437     |
| 0.0657      | 0           | 1.5128      | 4.2657    | 1.5679      | -0.0864     |
| 0.0968      | 0           | 1.4869      | 4.4251    | 1.5636      | -0.1267     |
| 0.1267      | 0           | 1.4629      | 4.5939    | 1.5581      | -0.1649     |
| 0.1695      | 0           | 1.4305      | 4.8631    | 1.5479      | -0.2182     |
| 0.1966      | 0           | 1.4112      | 5.0522    | 1.5402      | -0.2513     |
| 0.2376      | 0           | 1.3833      | 5.3687    | 1.5268      | -0.3003     |
| 0.2221      | 0           | 1.4014      | 5.9368    | 1.5338      | -0.2772     |
| 0.2129      | 0           | 1.4117      | 6.3180    | 1.5377      | -0.2637     |
| 0.2129      | 0           | 1.55467     | 6.2391    | 1.5682      | -0.2119     |
| 0.2198      | 0.021       | 1.55491     | 5.9493    | 1.5728      | 0.2183      |
| 0.2329      | 0.021       | 1.5554      | 5.4436    | 1.5729      | 0.2314      |
| 0.2376      | 0.021       | 1.5555      | 5.2756    | 1.573       | 0.2361      |
| 0.2325      | 0.0554      | 1.5707      | 5.2748    | 1.5264      | 0.2374      |
| 0.2203      | 0.1002      | 1.5545      | 5.2675    | 1.4983      | 0.2341      |
| 0.2168      | 0.1002      | 1.5676      | 5.2655    | 1.4854      | 0.2358      |
| 0.2111      | 0.1228      | 1.5524      | 5.2645    | 1.4817      | 0.2338      |
| 0.2017      | 0.1228      | 1.556       | 5.5535    | 1.4613      | 0.2271      |
| 0.1951      | 0.1228      | 1.5506      | 5.843     | 1.4596      | 0.2188      |
| 0.1869      | 0.1228      | 1.5442      | 6.2301    | 1.4577      | 0.2086      |

由上表得到的六个关节的逆解数据，采用五次多项式插值方法，将轨迹规划后运动量传递给跟踪控制程序。然后，通过CAN信号传输各个关节阀口的开度值给车载控制器，并使机械臂执行操作，以最终到达轨迹终点。在此过程中，数据采集的时间间隔为100ms。完成全自动控制实验后，我们绘制出六个关节的理论关节轨迹和实际关节轨迹的图，如图5-20至图5-25所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps20.jpg) 

图5-20 关节1的理论轨迹和实际轨迹

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps21.jpg) 

图5-21 关节2的理论轨迹和实际轨迹

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps22.jpg) 

图5-22 关节3的理论轨迹和实际轨迹

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps23.jpg) 

图5-23 关节4的理论轨迹和实际轨迹

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps24.jpg) 

图5-24 关节5的理论轨迹和实际轨迹

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps25.jpg) 

图5-25 关节6的理论轨迹和实际轨迹

分析上六张图可知，在前25 s时间喷嘴还未到达规划轨迹的起点，所给出的目标关节值可能过大且并未进行关节空间的轨迹规划，所以本次实验的跟踪控制分析主要考虑25 s-120 s区间内，对规划曲线的跟踪情况。关节1在跟踪关节轨迹的最大误差出现50 s-70 s，在此时间段平均误差为0.008 rad，其余时间段内跟踪性能较好，平均误差0.004 rad。关节2在跟踪关节轨迹的最大误差出现在60 s-80 s，平均误差为0.012 rad，其余时间段跟踪性能较好，平均误差为0.004rad。关节3的最大误差出现在60 s-70 s，平均误差为0.012 rad，其余时间段跟踪性能较好，平均误差为0.007 rad。关节4的最大误差出现在25 s-50 s，平均误差为250 mm，在其他时间段跟踪性能较好，平均误差为50 mm。关节5整个25 s-120 s时间段跟踪性能较好，平均误差在0.002 rad。关节6的最大误差出现在50 s-65 s，平均误差为0.015 rad，在其余时间段跟踪性能较好，平均误差为0.003 rad。

根据上述关节跟踪误差分析，关节5、6采用液压马达驱动并关节质量小，跟踪性能好。关节2、3、4是由液压缸驱动且质量较大，在运动启停阶段的惯性会使得传感器发生轻微转动和液压油缸的激振力也会让传感器读数发生变化。特别关节4是伸缩臂，但关节4运动时需要液压油量较大会影响其他关节阀口供油。关节1是液压马达驱动齿轮运动，由于齿轮间隙误差会导致关节1运动有偏差。在控制软件中，考虑到机械臂加工误差和外部干扰等，每个关节设置的位置精度为0.017rad，实验结果表明，该软件控制的机械臂误差都在0.017rad内，符合喷浆作业对喷浆机械臂的要求。

利用各关节变化量正运动学计算出实际末端轨迹和喷嘴理论运动轨迹对比，如图5-26的蓝色曲线是喷嘴理论运动曲线，红色曲线是喷嘴实际运动曲线。

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps26.jpg) 

图5-26 三维坐标系下的喷嘴轨迹图

三维坐标系下，喷嘴的理论和实际位置分别在X轴、Y轴、Z轴方向上跟踪误差如图5-27、图5-28、图5-29所示：

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps27.png) 

图5-27 X坐标的误差曲线

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps28.png) 

图5-28 Y轴的误差曲线

![img](file:///C:\Users\彭凯\AppData\Local\Temp\ksohtml24504\wps29.png) 

图5-29 Z轴的误差曲线

基于实验数据得到的机械臂各关节值，利用正运动模型求得喷嘴的运动轨迹。对比规划的理论喷嘴轨迹与实际喷嘴轨迹的误差，在X轴方向平均误差在0.03m内，在Y轴方向的平均误差为0.025m内，在Z轴方向的平均误差为0.015m内。因为自动化喷浆作业对末端喷嘴要求在0.1 m以内，上述实验结果表明，该控制系统实现末端控制精度符合喷浆作业行业要求。

综上所述，本文设计的喷浆机械臂自动控制系统能够有效使喷浆机械臂顺利完成喷浆工作，达到预期的控制效果。

## 5.4 本章小结

本章首先对机械臂控制软件设计，按照总软件架构进行功能模块化划分，详细阐述了**GUI模块、通讯模块、模式切换模块**的具体设计，以及机械臂定位和隧道模型导入界面。此外，机械臂控制软件将本文的运动学，轨迹规划和跟踪控制技术进行了程序化，有利于后续的现场喷浆机械臂实验。将喷浆机械臂控制软件控制机械臂作为实验对象，明确了实验目的和实验步骤。在改装喷浆机械臂基础上，以上位机、控制器、喷浆机械臂搭建实验平台。按照全自动实验流程有序开展现场实验，通过传感器得到机械臂的实际运动情况。将理论关节轨迹和实际关节轨迹1对比，关节1至关节6的平均误差在0.004rad、0.004rad、0.007rad、50mm、0.002mm、0.003mm，并分析了关节误差产生原因。喷嘴末端的理论轨迹和实际轨迹在X、Y、Z轴的平均误差为0.03m、0.025m、0.015m以内。以上实验数据表明，所设计的喷浆机械臂控制系统能够保证喷浆机械臂顺利运动，在精度和稳定性方面满足自动化喷浆机械臂的工程需求，具有较大的工程实用意义。
...
用过qt界面的哪些模块 有没有做过qt的重绘 qt信号槽跨线程用过没 怎么设置
...
①任意节点的任意时刻都可以发送数据，通讯方式灵活。

②每个节点都用不同的优先级，可以满足实时性要求。

③传输方式有点对点，点对多点，广播三种传送方式。

④在总线未被破坏的情况下，优先级高的先传送。

⑤通讯距离可达6km，数据传输速率最高为1MB，节点最多可以达到110个。
